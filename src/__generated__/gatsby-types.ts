...
type PostsFeedQueryVariables = Exact<{ [key: string]: never; }>;


type PostsFeedQuery = { readonly allMdx: (
    Pick<MdxConnection, 'totalCount'>
    & { readonly edges: ReadonlyArray<{ readonly node: (
        Pick<Mdx, 'id' | 'excerpt'>
        & { readonly frontmatter: Maybe<Pick<MdxFrontmatter, 'title' | 'date' | 'description'>>, readonly fields: Maybe<Pick<MdxFields, 'slug'>> }
      ) }> }
  ) };
